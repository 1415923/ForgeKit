...
5. 新项目填写 `docs/项目开发方案.md` 和 `docs/版本路线图.md`。
6. 接手项目填写 `docs/既有项目接手审计.md` 和 `docs/缺陷修复计划.md`。
7. 重要方案讨论写入 `docs/rfc/`，重要架构决策写入 `docs/adr/`。
8. 需求、RFC、ADR、任务、测试、缺陷关系写入 `docs/追踪矩阵.md`。
9. 高风险写入 `docs/风险登记册.md`。
10. 高影响变更写入 `docs/变更影响评估.md`。
11. 严重事故或重复缺陷写入 `docs/事故复盘.md` 或 `docs/缺陷复盘.md`。
12. 安全敏感变更写入 `docs/安全威胁建模.md`，依赖变更写入 `docs/依赖安全审查.md`。
13. 环境和发布链路写入 `docs/环境矩阵.md` 和 `docs/发布流水线.md`。
14. 模块负责人和评审责任写入 `docs/代码所有权.md`。
15. Epic、Feature、Task、Bug 写入 `docs/项目任务看板.md`。
16. 技术债写入 `docs/技术债记录.md`，版本质量写入 `docs/质量指标记录.md`。
17. 开始编码前，确认 Definition of Ready；完成时确认 Definition of Done。
