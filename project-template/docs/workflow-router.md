...
| loop / bounded-auto 授权 | `bounded-auto-loop-policy.md`, `ai-engineering-loop.md`, `agent-entry-contract.md`; native mode 时再读 `native-agent-adapter.md` | 仅在 confirmed fact 变化时写 responsible owner 或 active change/checkpoint | mandatory loop state、source/task/changelog docs unless their facts changed | authorization/effect envelope + stop + checkpoint/handoff |
...
| 归档后当前任务接不上 / task-intake 空了 / 看板和原始任务对不上 / current docs 断链 / archive 后任务跑偏 / 恢复当前状态 | `current-docs-integrity.md`、task-board、task-intake；仅在真实 fact 存在时读对应 risk/testing/traceability owner | 仅在用户确认后按 confirmed fact 做最小 Current State Restoration Pass | 从 active task 推导 mandatory population、继续 archive、全量复制 archive、覆盖 business docs | current-docs-integrity check + scoped restoration plan；不编造事实 |
...
- 涉及代码审查时，按 `maker-checker-protocol.md` 判断 independent gate 是否适用；适用时 Maker 用 `forgekit-request-code-review` 构造最小 packet 并调用 fresh read-only `forgekit-code-reviewer`。self-review 不得冒充已经要求的 independent review，reviewer 不可用时使用 `manual-review`。
- 用户要求第一性原理时执行 First-Principles Pass；用户要求对抗式审查时执行 Adversarial Review Pass。高风险设计前先推导根因和最小正确机制，高风险收口前先找失败路径。
- 未验证推导不得写成事实；`Blocking=YES` finding 必须进入 checkpoint 并停止其 BlockedScope。`TODO_REVIEW` 按 ValidationRelevance 和当前 scope 处理，不自动扩大为 Blocking。
