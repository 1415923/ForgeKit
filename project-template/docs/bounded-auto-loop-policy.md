...
Checkpoint 由授权中声明的事件和真实事实变化触发，不按每个 stage 固定生成 work-log，也不要求 state file。按 `context-continuity.md` 与 `work-session-checkpoint.md` 执行 writeback check；无新事实时不写。关键结论不能只留在聊天里；长工具输出只保留摘要、证据路径和 `TODO_REVIEW`，不得写入全文。`minimal` 只在对应事实确实变化时更新唯一负责 owner，可包含：
