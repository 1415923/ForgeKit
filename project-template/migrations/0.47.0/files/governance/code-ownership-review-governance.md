...
| 文档 | 用途 |
| --- | --- |
| `.forgekit/docs/code-ownership.md` | 记录模块、负责人、评审人、风险等级和替补人 |
| `.codex/git.md` | 记录分支、提交和合并前要求 |
| `.forgekit/docs/change-impact.md` | 记录跨模块或高风险改动的影响 |

<!-- forgekit:section s-49ed252f40bb -->
## 所有权等级
...
<!-- forgekit:section s-dcea0c868ca6 -->
## CODEOWNERS 风格规则

项目可以在 `.forgekit/docs/code-ownership.md` 中用类似格式记录：
...
<!-- forgekit:section s-b219a209aed3 -->
## 必须升级确认的改动
...
- 修改前先识别影响路径，并对照 `.forgekit/docs/code-ownership.md`。
- 如果命中 Critical 或 Unknown 区域，应提醒用户需要负责人或人工确认。
- 如果缺少 `.forgekit/docs/code-ownership.md`，接手项目先建立初版所有权矩阵，不把未知区域当作可自由重构区域。
- 代码审查时检查是否遗漏必要评审人、影响范围或变更影响评估。
- 发布前检查 Critical 区域变更是否已有评审结论。

<!-- forgekit:section s-d1a64a237669 -->
## 接手项目特殊规则
