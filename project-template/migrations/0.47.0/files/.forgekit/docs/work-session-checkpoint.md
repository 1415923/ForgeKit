...
<!-- forgekit:section s-5e875c4b21ae -->
## Writeback Levels
...
<!-- forgekit:section s-1fbbce91114f -->
## Write Targets
...
<!-- forgekit:section s-e23334ba9d72 -->
## Do Not Write Back
...
<!-- forgekit:section s-585848e32d81 -->
...
<!-- forgekit:section s-8383e4ced245 -->
## 委派和中断恢复

父会话保留关键根因、证据路径、阻断范围和下一步，不复制完整子 agent 输出或工具日志。独立 reviewer 保持只读，self-review 不能满足独立门禁。未知项标记 TODO_REVIEW。用户途中补充默认修正当前任务，除非明确取消目标。

仅运行时支持时使用异步工具或委派；ForgeKit 不模拟后台执行。读取结果后再执行依赖该结果的操作。

<!-- forgekit:section s-4f2b9c60d93f -->
## 升级后的会话

更新入口、Skills 或 agents 后，当前会话仅最小保存进展并收口；新任务新开会话或重启工具。磁盘文件更新不证明运行时已重新加载。
