...
changed_paths() {
  ...
  } | awk 'NF' | grep -v '^.forgekit/upgrade-export/' | grep -v '^.forgekit/archive/' | grep -v '^.forgekit/archive-plan.md$' | grep -v '^.forgekit/archive-apply-report.md$' | grep -v '^.forgekit/archive-reference-report.md$' | sort -u
}
