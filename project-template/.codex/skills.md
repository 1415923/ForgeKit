...
- 使用 skill 前先阅读其说明。
- skill 不能覆盖本项目的明确规则。
- 使用 skill 前先通过 `AGENTS.md` 和 `docs/codebase-map.md` 确认入口，不要让 skill 触发全量读取。
...
- `docs/codebase-map.md` 负责告诉 skill 从哪里开始找代码。
...
- 初始化后方向不清楚：先读 `docs/codex-next-work-order.md`，必要时运行 `scripts/detect-local-toolchain.ps1` 和 `scripts/run-harness-check.ps1` 作为只读事实检查。
