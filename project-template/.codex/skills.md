...
ForgeKit v0.12.0 起采用根级统一 plugin 表面：`.codex-plugin/`、`.claude-plugin/` 和共享 `skills/` 位于仓库根。plugin 不应包含个人 `user-rules/`、本机路径、凭据或外部开发记录，也不应默认启用 hooks、MCP 或外部写操作。团队引入或升级 plugin 前，先按 `governance/team-agent-rollout.md` 做权限和维护责任审查。
