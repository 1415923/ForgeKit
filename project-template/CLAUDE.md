...
4. Read `.forgekit/docs/local-toolchain.md` when lint, test, build, or local validation commands matter.
...
8. Use `.claude/skills/` for ForgeKit workflow skills.
9. Read `governance/agent-harness.md` when the task involves context strategy, large code search, or unclear entry points.
...
14. Read `governance/agent-suitability.md` for initialization, existing project handover, or when project fit is unclear.
...
- Before loop one-step, restate the blueprint fields: loop name, state file, allowed paths, forbidden paths, validation command, stop condition, human escalation, token/scope budget, and whether files will change.
