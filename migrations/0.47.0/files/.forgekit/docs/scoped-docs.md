...
<!-- forgekit:section s-5dd70bd4741e -->
## Boundary Model
...
<!-- forgekit:section s-69f9430b0ae1 -->
## Scope Selection
...
<!-- forgekit:section s-d2b2597b595d -->
## Docs Profiles
...
<!-- forgekit:section s-7daf64cd7868 -->
## Project Capsule
...
### Minimal Project Capsule Bootstrap

只有用户已经启用 scoped docs，并在 `workspace-map.json` 中把一个 project 明确设为 `docs_profile: "project-capsule"` 后，才可创建最小 capsule：
...
- 第一版一次只处理一个 project，不支持 `--all` 或 `--force`。
- `plan` 只显示 `create`、`already-present`、`review-needed` 和 `blocking`，不写文件。
- `apply` 必须显式 `--confirm`，且只从 `.forgekit/projects/_template/` 创建六个最小文件。
- `workspace-only` 返回 `skipped_workspace_only`，不会创建 capsule 或 warning。
- 已有同内容文件视为 no-op；已有不同内容文件标记 `review-needed`，保留原文件并继续安全创建其他缺失文件。
- 脚本不修改 workspace map、Workspace Docs、repo、artifact、archive 或 Git 状态。完成后运行 `scripts/check-workspace-integrity.py` 检查跨层关系。

<!-- forgekit:section s-3064189ef8ae -->
## Source Rules
...
<!-- forgekit:section s-2cc4605f86c3 -->
## Status Rules
...
<!-- forgekit:section s-248181643228 -->
## Decision Records
...
<!-- forgekit:section s-5d939d40080b -->
## Activation
...
<!-- forgekit:section s-b0079f0f7448 -->
## Boundaries

- 不自动创建 ProjectScope、RepoRoot 或 ArtifactRoot。
- 不自动启用 map、切换 docs profile、批量创建 capsule 或迁移 Source/Task/Risk/Test。
