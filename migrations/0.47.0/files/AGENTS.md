...
定位不清或接手项目时使用 `.forgekit/docs/codebase-map.md`；定位已明确时直接读取相关实现。Skill 实现在 `.agents/skills/<skill>/SKILL.md`；仅加载选中技术栈 `.codex/stacks/<stack>/`。
...
整份定制入口的升级冲突按 `.forgekit/docs/project-maintenance.md` 复核并显式合并。
