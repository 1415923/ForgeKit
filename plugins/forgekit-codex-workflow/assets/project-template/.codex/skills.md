...
ForgeKit v0.9.0 提供 `forgekit-codex-workflow` plugin，用于把稳定项目治理 skills、模板资产和只读检查脚本分发给团队。plugin 不应包含个人 `user-rules/`、本机路径、凭据或外部开发记录，也不应默认启用 hooks、MCP 或外部写操作。团队引入或升级 plugin 前，先按 `governance/team-agent-rollout.md` 做权限和维护责任审查。
