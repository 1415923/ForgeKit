...
<!-- forgekit:section s-7091e34466e2 -->
...
<!-- forgekit:section s-748e0c7c5ae6 -->
...
<!-- forgekit:section s-16b94436c167 -->
...
<!-- forgekit:section s-0bfe26dd677c -->
...
<!-- forgekit:section s-f8481d2e54ba -->
...
<!-- forgekit:section s-cd26379910aa -->
...
<!-- forgekit:section s-8b5f820c46ee -->
## Skill Routing
...
用户明确授权的结果包含必要的相关实现、验证及修复；在已批准阶段内不重复询问。澄清只针对会改变结果或授权边界的缺口，继续不依赖该缺口的工作。不要在第一版实现后无故停止。
用户明确指令优先于 Skill 指导；系统和工具权限不受项目文件降低。因规则停止时指出文件、具体规则、被阻塞动作及这是明文要求还是解释。
必需验证通过后，只有新变更、失败或未解决疑点才扩大或重复测试。按实际结果报告，不新增假设性门禁。
