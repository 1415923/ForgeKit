...
This is the lightweight Codex entry for the generated project. The unique shared safety, evidence, authorization, writeback, and routing contract is [`governance/agent-entry-contract.md`](governance/agent-entry-contract.md); this file applies that contract and does not replace it.
...
3. Under the [Skill Routing](governance/agent-entry-contract.md#skill-routing) contract, load the matching project-local `.agents/skills/<skill>/SKILL.md`, then only its direct references. Load only the selected `.codex/stacks/<stack>/` material.
...
| Intent | Route |
| --- | --- |
| Initialize, fill confirmed setup facts, or assess fit | `project-init`, `project-bootstrap-fill`, `project-suitability` |
...
| Check release readiness | `release-check` |
| Project maintenance, archive, handoff, loop, worktree, or scoped-doc operations | `.forgekit/docs/workflow-router.md`, then the named Skill or governance file |

Specific Skill triggers and execution contracts belong to each Skill. In particular, use `code-review` for independent code review, `handover-review` for takeover audit, `release-check` for release checks, and `large-change-planning` for high-impact planning; this entry does not redefine those workflows.
