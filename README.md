...
| ForgeKit 升级 | `.forgekit/upgrade/upgrade-plan.md`、`.forgekit/upgrade/upgrade-actions.md`、`.forgekit/upgrade/candidates/<version>/` |
...
生成配置不等于 runtime 已注册。实际运行时必须能观察到 `forgekit-planner`、`forgekit-reviewer` 或 `forgekit-verifier` 被调用，才能记录为 native；如果走 general-purpose / worker 加 prompt injection，只能记录为 fallback，不能说 native 成功。
...
从旧版本 ForgeKit 升级到新版本时，优先使用 guided upgrade，不要用 `-Force` / `--force`：
...
guided upgrade 只生成计划和候选模板，不覆盖项目文件：

- 保留已有项目事实、`.forgekit/template-lock.json`、business `docs/`、源码、`.codex/`、`AGENTS.md`、`CLAUDE.md`。
- 生成 `.forgekit/upgrade/upgrade-plan.md`，按 must_review / merge_carefully / can_add / can_ignore / template_only 分类。
- 生成 `.forgekit/upgrade/upgrade-actions.md`，给 AI 一个短的升级合并任务单。
- 把新版候选模板按目标路径导出到 `.forgekit/upgrade/candidates/<version>/`。
- 旧项目缺少 `.forgekit/template-lock.json` 时生成 `.forgekit/upgrade/legacy-inventory.md`，不自动补 lock。

升级后可让 AI 执行：

```text
请读取 .forgekit/upgrade/upgrade-actions.md，按 .forgekit/upgrade/upgrade-plan.md 合并 ForgeKit 升级内容。只处理 must_review、merge_carefully 和需要的 can_add；不要覆盖项目真实事实，不要修改 business docs、源码、template-lock，也不要把 candidates 当作当前态文档。
```
