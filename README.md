...
任务来源优先要求领导任务、微信任务、计划表格子、会议任务或手工记录先写入 `.forgekit/docs/task-intake.md`：保留脱敏后的原文、责任拆分、时间窗口、AI 理解、拆解任务 ID 和 `Human Review` 状态。`requirements.md` 记录需求事实，`task-board.md` 记录拆解后的任务状态并反链 Source ID，`changelog.md` 记录完成后的版本变化；它们都不替代任务派发原文台账。
