...
| `.forgekit/docs/document-responsibility.md` | managed docs 职责矩阵：Audience、触发条件、写什么、不写什么、默认读取策略 |
...
| `.forgekit/docs/task-intake.md` | 任务派发原文台账：先保留领导/微信/计划表/会议原文，再做 AI 分析和任务拆解 |
...
Source-First Task Intake 要求领导任务、微信任务、计划表格子、会议任务或手工记录先写入 `.forgekit/docs/task-intake.md`：保留脱敏后的原文、责任拆分、时间窗口、AI 理解、Derived Task IDs 和 Human Review 状态。`requirements.md` 记录需求事实，`task-board.md` 记录拆解后的任务状态并反链 Source ID，`changelog.md` 记录完成后的版本变化；它们都不替代任务派发原文台账。

Managed Docs Responsibility Matrix v2 把 `.forgekit/docs/**` 分成 core、current、working、triggered、reference、generated、archive。AI 默认先读 `document-responsibility.md` 和 `codebase-map.md`，再按任务触发读取相关文档；不要全量读取 docs，也不要把同一事实重复写进多个文档。
...
| 版本 | 用户可感知变化 |
| --- | --- |
| `0.28.2` | Managed Docs Responsibility Matrix v2：收紧 managed docs 职责、默认读取策略和重复写入边界，让用户可读文档更短、更自然。 |
| `0.28.1` | Source-First Task Intake：新增 `.forgekit/docs/task-intake.md`，保留任务派发原文、AI 分析、任务反链和人工确认状态。 |
