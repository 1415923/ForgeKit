...
这一个命令会完成项目模板初始化，并生成 Claude Code / Codex 可审查的 Native Agent Adapter 配置模板。生成配置不等于 runtime 已注册，首次使用仍要按 `.forgekit/docs/native-agent-adapter.md` 验证是否真的调用到 `forgekit-*` agent。
...
如果已有代码，把整个业务项目放进内层 `my-app/`；如果是新项目，后续源码、测试、业务 README、构建文件也都放在内层。需要推送时，只在内层 `my-app/` 初始化 Git、commit 和 push，避免把外层 ForgeKit 治理文档和配置一起推送到业务仓库。
...
新用户不需要一次读完所有文档。先按场景选择入口：

| 场景 | 主要读取 |
| --- | --- |
| 个人独立开发新项目 | `AGENTS.md` / `CLAUDE.md`、`.forgekit/docs/document-responsibility.md`、`.forgekit/docs/codebase-map.md`、`.forgekit/docs/workflow-router.md`、`.forgekit/docs/task-intake.md`、`.forgekit/docs/task-board.md`、`.forgekit/docs/work-log.md` |
| 公司项目接手 | `AGENTS.md` / `CLAUDE.md`、`.forgekit/docs/document-responsibility.md`、`.forgekit/docs/codebase-map.md`、`.forgekit/docs/workflow-router.md`、`.forgekit/docs/local-toolchain.md`、`.forgekit/docs/handover-audit.md`、业务 `README` / `docs/` |
| 日常任务开发 | `.forgekit/docs/task-intake.md`、`.forgekit/docs/task-board.md`、`.forgekit/docs/work-log.md`，按需读取 `testing.md` / `requirements.md` / `architecture.md` |
| 中高风险变更 | `.forgekit/changes/<change-id>/`、`.forgekit/docs/testing.md`、`.forgekit/docs/changelog.md`，必要时读取 `architecture.md` |
| 发布前检查 | `.forgekit/docs/changelog.md`、`.forgekit/docs/testing.md`、`.forgekit/docs/task-board.md`、`.codex/version-gates.md` |
| ForgeKit 升级 | `.forgekit/state.json`；依次运行 `forgekit-upgrade.py check`、`plan`、`apply --safe` |

接手阶段的审计文档不是日常任务入口。项目稳定后，日常工作通常围绕 `task-intake -> task-board -> work-log`，再按事实变化更新需求、测试、架构和 changelog。

业务代码默认位于内层 `ProjectName/`。AI 需要读代码时进入或引用内层目录；ForgeKit managed docs、AGENTS、CLAUDE、adapter 配置留在外层治理目录。
...
| `.forgekit/docs/workflow-router.md` | 用户意图路由表：判断一句话该读什么、写什么、不要写什么 |
| `.forgekit/doc-health-report.md` | 文档健康报告：检查 managed docs 过长、重复、职责错位和 router 边界风险；report-only |
| `.forgekit/source-trace-report.md` | 来源追溯报告：检查 Source ID、需求、任务、change、验证、work-log、changelog 链路断点；report-only |
| `.forgekit/handoff-package.md` | Review-ready 交付包：汇总来源、任务、变更、验证、风险和 TODO_REVIEW；report-only |
...
| `.forgekit/docs/loop-operations.md` | 显式触发的 loop dry-run、one-step、bounded-auto、review-only、continue、stop/handoff 操作协议，不是自动 runner |
| `.forgekit/docs/bounded-auto-loop-policy.md` | 有限授权连续推进的 scope、stages、budget、stop conditions、agent mode 和 handoff 规则 |
...
| `workflow-router` 文档 | 把“看领导原文、当前任务、验证结果、今天做了什么、准备汇报、继续 loop、收口版本”等请求路由到正确文档 |
| `doc-health-report.py` | 生成 `.forgekit/doc-health-report.md`，汇总文档过长、重复、职责错位和 workflow-router 边界风险，不自动修复 |
| `source-trace-report.py` | 生成 `.forgekit/source-trace-report.md`，检查来源到任务、实现、验证和状态记录的追溯链路，不自动补链 |
| `handoff-package.py` | 生成 `.forgekit/handoff-package.md` 或 change scoped `handoff.md`，把阶段成果整理成给领导、reviewer、测试可读的交付包，不编造缺失证据 |
...
Bounded Auto Loop Policy 只定义三种用户显式授权模式：`one-step` 每轮后停、`bounded-auto` 在 scope / stages / budget / stop conditions 内推进、`review-only` 只审查不写文件。它不提供 runner、daemon、cron、scheduler、自动 PR 或 worktree 编排。

Optional Loop Operation Mode 只定义用户显式触发的 dry-run、one-step、bounded-auto、review-only、continue、stop/handoff 协议；loop 默认关闭，不提供后台自动化、无人值守 runner 或连续循环。

Maker / Checker Protocol 现在使用 Independent Code Review Protocol：Maker 通过 `forgekit-request-code-review` 只发送任务摘要、diff/stat、changed files、验证输出和已知风险；独立 `forgekit-code-reviewer` 保持 read-only，并输出 pass / needs-fix / manual-review。self-review 不能冒充 independent review；reviewer 不可用时必须 manual-review。该协议不自动修复、不调用 PR API，也不是自动多 agent 系统。
...
Managed Docs Writeback Policy 默认使用 `minimal`：业务实现范围与 ForgeKit 治理写回范围分开判断。即使用户要求只改指定业务文件，任务完成后仍可按事实最小更新 `work-log.md`、状态确实变化的 `task-board.md`、用户/版本可见变化的 `changelog.md` 和当前 change；只有用户明确禁止文档写入时才关闭。`review-only` 和 report-only 脚本仍不写或自动修复 managed docs。
...
Native Agent Adapter 是 opt-in 功能，用来把 ForgeKit 的 loop / maker-checker / verification 协议导出为 Claude Code / Codex 可审查的原生 agent 配置模板。它只生成配置，不执行 loop，不启动 agent，不做 runner、dispatcher、worktree 自动化、merge、commit、push 或 PR。

生成配置不等于 runtime 已注册。实际运行时必须能观察到对应的 `forgekit-*` agent（包括 `forgekit-code-reviewer`）被调用，才能记录为 native；如果走 general-purpose / worker 加 prompt injection，只能记录为 fallback，不能说 native 成功。
...
旧的 `generate-native-agent-adapter.*` 单独脚本保留为兼容和维护入口，不作为新用户主路径。新项目优先使用初始化命令一次完成。

Codex adapter 会生成 `.codex/agents/*.toml` 和最小项目级 `.codex/config.toml`。先运行 `python scripts/check-codex-native-agents.py --repo-root .` 检查 schema；`SchemaStatus: pass` 仍不等于 runtime 已注册。只有实际观察到 `forgekit-*` 被调用，才记录为 native；如果 Codex 只显示 default / explorer / worker，记录为 `native_agent_status=unavailable`。
...
v0.36.0 起采用 Versioned Migration Upgrade Model。新项目只需理解三步：
...
- `check`：检查 `.forgekit/state.json` 和当前版本是否支持迁移，不改文件。
- `plan`：输出一屏可读的版本迁移计划，不改文件。
- `apply --safe`：只执行 migration 中明确标记为 safe 的动作；不做三方 merge，不改 business docs，不自动提交。

只有 v0.36.0 及以后初始化、并具有 `.forgekit/state.json` 的项目支持该模型。`Pre-v0.36 projects`（v0.35.x 及更早项目）统一按“接手既有项目”处理：先盘点当前事实，可按需生成 handoff、doc-health、source-trace 报告；用户确认 adoption 边界后，才建立新的 v0.36+ state。ForgeKit 不自动迁移旧 docs，也不假装旧项目已自动升级。

旧 `upgrade-forgekit.ps1` / `.sh` / `.py` 仅作为 legacy report-only 兼容入口保留，不再是推荐路径。不要默认读取或导出全量 candidates / upgrade-export。
...
| 版本 | 用户可感知变化 |
| --- | --- |
| `0.37.0` | Independent Code Review Protocol：新增独立只读 code reviewer、request/review skills、最小上下文包和 pass / needs-fix / manual-review gate。 |
| `0.36.0` | Versioned Migration Upgrade Model：新项目使用 state + migration 的 check / plan / apply --safe 三步升级；v0.35.x 及更早项目改走既有项目 adoption。 |
| `0.35.2` | Managed Docs Writeback Policy：区分业务实现范围与治理写回范围，恢复任务完成后的最小 work-log / task-board / changelog / change 写回，同时保持 review-only 与 report-only 边界。 |
| `0.35.1` | Codex Custom Agent Schema Hotfix：修正并校验 Codex custom agent TOML，确保 `name`、`description`、`developer_instructions` 都是字符串，避免 table/map schema 报错。 |
| `0.35.0` | Review-Ready Handoff Package：新增 report-only 交付包，汇总来源、需求、任务、变更、验证、风险和 TODO_REVIEW，不自动修复、不提交、不创建 PR。 |
| `0.34.0` | Source Trace Report v2：新增 report-only 来源追溯报告，检查 Source ID -> requirement -> task -> change -> verification -> work-log/changelog 链路断点，不自动修复。 |
| `0.33.0` | Doc Health Report v2：新增 report-only 文档健康汇总，检查 managed docs 过长、重复、职责错位和 workflow-router 边界风险，不自动修复。 |
| `0.32.0` | Workflow Router：新增 `.forgekit/docs/workflow-router.md`，把用户意图路由到正确 Read / Write / Do Not Write 文档，减少全量读取和重复写入。 |
| `0.31.0` | Bounded Auto Loop Policy：新增 one-step / bounded-auto / review-only 授权边界、预算、停止条件、agent mode 门禁和 handoff 规则，不提供 runner。 |
| `0.30.1` | Native Agent Adapter Verification：明确 generated config 不等于 runtime registered，新增 native / fallback / simulated 记录字段和验证清单。 |
| `0.30.0` | Native Agent Adapter：新增 opt-in 原生 agent 配置生成，把 ForgeKit loop / maker-checker / verification 协议导出为 Claude Code / Codex 可审查模板，不提供自动执行。 |
| `0.29.0` | Guided Upgrade Workflow：新增独立升级脚本，生成 upgrade-plan、upgrade-actions 和候选模板，减少旧项目升级时的人工判断和 token 消耗。 |
