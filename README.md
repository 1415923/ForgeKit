...
不确定时先选 `Standard`。`Mode` 只写入初始化 metadata，用于后续 AI 填充优先级和治理强度讨论；v0.14.0 不按模式裁剪复制文件。不要在这一步急着选择技术栈；新项目的技术栈应在方案讨论后确定，既有项目应先由 AI 扫描现有文件推断。
...
- `docs/codebase-map.md`：代码入口、模块地图和局部验证命令。
- `docs/local-toolchain.md`：本地 lint、test、build、LSP 和工具链能力。
