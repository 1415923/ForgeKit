...
长会话、上下文压缩、清空会话、子 agent 委派和 bounded-auto 多轮推进后，关键工程结论可能只存在聊天里。后续会话无法可靠恢复用户确认的范围、bug 根因、验证结论、失败路线和 reviewer 阻断，只能重复排查或重新猜测。

### 设计结论

聊天上下文不是可靠工程状态。ForgeKit 应在阶段边界和上下文切换前执行最小 checkpoint，把关键事实写入职责最窄的 managed doc 或 change artifact；完整聊天、临时推理和长工具输出不进入常驻文档。

### Added

- 新增 `.forgekit/docs/context-continuity.md`，定义 Critical Facts、Checkpoint Triggers、Context Survival Map 和 compact/clear readiness。
- 新增长会话、子 agent 结论、大工具输出和 handoff 前的最小 checkpoint 规则。
- handoff package 增加 Context Continuity Check，缺少关键证据时标记 `TODO_REVIEW`。
- 新增 v0.37.0 到 v0.38.0 的 safe migration：只安装缺失的协议文档并登记 feature，已有入口规则保持人工合并。

### Changed

- workflow-router 增加“保存关键结论 / compact 或 clear 前 checkpoint / 子 agent 返回 / 长会话续接”路由。
- bounded-auto 每个 checkpoint 先检查关键事实是否已落盘，再执行最小 managed docs writeback。
- AGENTS、CLAUDE 和 Codex rules 明确：关键结论不能只留在聊天里，长输出只保留摘要和证据路径。

### 边界

- 不自动执行 compact 或 clear。
- 不新增 token monitor、hook、runner、daemon 或后台调度。
- 不自动读取全量 managed docs。
- 不把完整聊天、长日志或所有项目事实写进 `CLAUDE.md` / `AGENTS.md`。
- 不自动修改未经用户确认的 requirements 或 task-intake 原文。
