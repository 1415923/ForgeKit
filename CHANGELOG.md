...
## [0.42.0] - Project Capsule Bootstrap

### 问题背景

v0.41.0 引入了 Multi-Project Workspace & Scoped Docs Protocol，ForgeKit 已经能用 `.forgekit/workspace-map.json` 描述 Workspace、Project、Repo、Artifact 和 Archive 的边界，也能通过 `check-workspace-integrity.py` 检查跨层路径、profile、artifact/repo 混用和 capsule 过重等问题。v0.41.1 又补齐了 `workspace-only` / `project-capsule` / `repo-lite` 的语义，使用户可以先启用 workspace map，而不必立刻为所有项目创建 capsule。

但真实试用后，新的问题变得很明确：**v0.41 能检查项目是否应该有 capsule，却没有提供一个安全、最小、可重复的方式创建 capsule**。

如果让用户或 AI 手动复制 `.forgekit/projects/_template/`，容易出现几类风险：

* 复制错路径，把 Project Capsule 放到业务 repo 或 artifact 目录里。
* 复制过多，把完整 ForgeKit docs、AGENTS、CLAUDE、workflow-router 等治理入口也复制到项目局部目录。
* 忘记建立 `project-card.md`、`source-links.md`、`task-board.md`、`testing.md`、`risk-register.md` 和 `decisions/` 的最小集合。
* 把 workspace 原始 source、完整 task-board 或历史 risk 直接拆进 project capsule，导致事实重复和断链。
* 多个 project 要创建 capsule 时，没有统一的 dry-run、已存在文件处理和 review-needed 提示。

这些问题说明：v0.42 不应该马上做自动拆文档、状态聚合或 repo-lite 写入，而应该先补上一个最小的 **Project Capsule Bootstrap** 能力。

### 设计结论

v0.42.0 将 Project Capsule 的创建从“手动复制模板”收敛为一个安全 bootstrap 操作。

这版的核心结论是：

* **workspace-map 是输入，不是副作用结果**：bootstrap 只读取已启用的 `.forgekit/workspace-map.json`，不会替用户猜 project/repo/artifact，也不会自动启用 multi-project。
* **只为 `docs_profile: project-capsule` 的 project 创建 capsule**：`workspace-only` 表示项目事实仍由 Workspace Docs 管理，不应创建 capsule；`repo-lite` 只属于 repo，不生成项目文档。
* **bootstrap 只创建空的最小局部文档，不迁移真实事实**：Project Capsule 初始只包含模板和少量 project metadata，不自动拆分 `task-board.md`、`risk-register.md`、`testing.md`、`work-log.md` 或 source 原文。
* **已存在内容优先保留**：目标 capsule 文件存在时，不覆盖；内容相同时 no-op；内容不同则标记 review-needed。
* **Project Capsule 不是第二套完整 ForgeKit**：capsule 只能包含局部事实文件，不得复制完整 managed docs、入口规则、skills、agents、archive protocol 或 workflow router。
* **先 dry-run，再 apply**：会写文件的 bootstrap 必须先输出计划；apply 需要显式确认。

### Added

* 新增 Project Capsule Bootstrap 能力，用于根据 `.forgekit/workspace-map.json` 为 `docs_profile: project-capsule` 的 project 创建最小 capsule。
* 新增 dry-run 计划输出，列出将创建的 project capsule、目标路径、文件清单、已存在文件和 review-needed 项。
* 新增 apply 确认流程，只在用户显式确认后创建缺失的 capsule 文件。
* 新增 capsule 创建的幂等规则：

  * 目标文件不存在：创建；
  * 目标文件存在且内容相同：no-op；
  * 目标文件存在但内容不同：保留用户文件，标记 review-needed；
  * 目标路径类型错误或越界：blocking。
* 新增 bootstrap 回归测试：

  * `workspace-only` project 不创建 capsule；
  * `project-capsule` project 创建最小 capsule；
  * 已存在 capsule 不覆盖；
  * artifact / archive / repo 路径不能被当作 capsule 目标；
  * capsule 不得包含完整 ForgeKit 入口或治理副本；
  * dry-run 不写文件，apply 需要确认。

### Changed

* workspace scoped docs 流程从“只定义和检查 project capsule”扩展为“可以安全创建最小 project capsule”。
* workflow-router 增加 Project Capsule Bootstrap 路由：

  * “给这个 project 建 capsule”
  * “根据 workspace-map 初始化 project docs”
  * “为 `project-capsule` 项目创建最小文档”
  * “只生成计划，不要写文件”
* project maintenance 文档增加 scoped workspace 下的 capsule 初始化流程。
* README / README.en / usage.html 增加最小示例，说明如何从 `workspace-only` 过渡到 `project-capsule`。
* `check-workspace-integrity.py` 继续保持只读；bootstrap 不替代 checker，bootstrap 后仍应运行 workspace integrity check。

### 用户可感知变化

* 用户可以先在 `.forgekit/workspace-map.json` 中把某个 project 从 `workspace-only` 改为 `project-capsule`，再通过 bootstrap 创建最小 capsule。
* 用户不需要手动复制 `_template` 目录，也不需要记住 capsule 应该包含哪些文件。
* 创建 capsule 前可以先看到计划，确认哪些文件会被创建、哪些已经存在、哪些需要人工 review。
* 已有 project capsule 不会被模板覆盖。
* 创建完成后，用户可以运行 workspace integrity check 验证 capsule 是否过重、路径是否安全、profile 是否匹配。
* 多项目工作区可以逐步启用 capsule：先给后端创建，后续再给前端或 scanner 创建，不需要一次性拆完整个 workspace。

### 边界

* 不自动启用 multi-project scoped docs。
* 不自动修改 `.forgekit/workspace-map.json` 的 `enabled` 状态。
* 不自动把 `workspace-only` project 改成 `project-capsule`。
* 不自动创建未登记在 workspace map 中的 project。
* 不自动拆分 workspace `task-board.md`、`risk-register.md`、`testing.md`、`work-log.md` 或 `task-intake.md`。
* 不迁移 Source / Task / Risk / Test 记录。
* 不复制 workspace Source 原文到 project capsule。
* 不生成 Repo Lite 文件。
* 不移动业务代码、repo、artifact 或 archive。
* 不自动 commit、push、PR 或 tag。
* 不新增 runner、daemon、scheduler、后台任务、文档站点或 UI。
* 不把 Project Capsule 变成完整 ForgeKit 副本；capsule 仍然是最小局部事实集。
