...
本文件记录 ForgeKit 的用户可感知变化、升级注意事项和重要边界。使用方式见 [README.md](README.md) / [README.en.md](README.en.md)。
...
- 面向 GPT-6 Astra 精简重复入口、授权和验证指导，保留 Codex / Claude 双平台，不修改用户模型配置。
- 收敛为七个主要 Skill；初始化填充与事实回填合并，适用性与接手评估合并，按需加载 references。三个旧名称在 0.47.x 保留为显式兼容入口。
- 合并重复文档：连续性归入 work-session-checkpoint，工具链归入 testing，意图路由归入 usage-playbook；搬迁退休 .codex 文档的定制内容。退休 usage.html。

### 保留定制的版本迁移

- 新增 0.46.0 → 0.47.0 结构化迁移，保留历史 migration。统一入口展示完整迁移链，先在内存中逐步规划，整链无冲突后应用；冲突不会推进项目版本。
- 引入 Markdown 章节标识、用户区、三方合并、输入与计划哈希、实际结果报告和回滚备份。项目事实文档使用明确白名单，可以保留自身章节结构。
- 修复历史逐文件基线与迁移素材识别、LF / CRLF / BOM 等价处理、跨版本入口来源标识重复，以及 Windows 中文 JSON 管道乱码。
- 整份重写的入口继续要求显式处理，错误行号说明为首个基线差异。可经授权整理为模板区＋项目用户区后正常升级；另提供绑定项目、版本和内容哈希的 `--entry-resolutions`，用于经过复核的完整保留方案。
- 普通升级更新 AGENTS / CLAUDE，无需手工粘贴新规则；升级后再次运行返回 up-to-date、零动作。项目专用合并材料保持本地，不进入共享模板。

### 文档与验证边界

- 中英文 README 对齐安装、迁移链、定制入口维护与事实归属：模板规则随版本更新，项目约束保留在用户区，易变运行状态引用对应证据；完整保留原文不代表业务事实已刷新。
- 静态契约、回归测试、生成项目 smoke、定制入口及历史版本迁移场景已验证；实际项目通过普通统一入口完成升级。
- Astra 实际行为、客户端加载和性能收益仍需真实客户端证据；本版没有据静态检查宣称 token、耗时或成功率改善。
...
v0.43.1 收口了 review-needed 升级闭环、manual-merge 和 CLI 中英文提示，但发布后发现部分版本元数据、检查脚本或模板状态仍指向 0.43.0，导致工具可能把 v0.43.1 误判为 0.43.0，或把升级目标错误地停在 0.43.0。

### 设计结论

v0.43.2 不新增功能，只修复 ForgeKitRoot、project-template、migration、manifest、metadata、validation 和 smoke 对当前版本的共同认知。v0.43.1 标记为 withdrawn / superseded，不再作为推荐稳定版本。

### Fixed

- 修复 latest/current/template version 仍指向 0.43.0 的问题。
- 修复新初始化项目 state 版本不正确的问题。
- 修复检查脚本 / smoke 对当前版本判断不一致的问题。
- 修复 migration chain 无法可靠收敛到当前版本的问题。
- 明确 v0.43.1 已被 v0.43.2 取代。

### 边界

- 不改 Project Capsule Bootstrap 语义。
- 不改 review-needed manual-merge 语义。
- 不新增 CLI 命令。
- 不新增 --force。
- 不改 workspace-map。
- 不创建 capsule。
...
## [0.43.1] - Withdrawn / Superseded

### 状态

该版本不建议作为稳定版本使用，已由 v0.43.2 取代。

### 原因

v0.43.1 包含 review-needed 升级闭环、manual-merge 和 CLI 中英文提示修复，但部分版本元数据、检查脚本或模板状态仍指向 0.43.0，可能导致工具自我识别和升级目标错误。
...
- review-needed 升级闭环：safe migration 遇到本地文件和模板基线不一致时，不再只输出 warning，而是在同一轮流程里解释原因、展示影响、让用户选择 `r/m/d/a`，并继续完成升级或明确中止。
- manual-merge：保留本地文件的同时导出新版模板、diff 和 README（含 AI 辅助合并提示词），方便后续人工或 AI 合并；`keep-local` 保留为兼容别名。
- CLI 中英文提示：新增 `--lang zh-CN` / `--lang en-US` 参数和 `FORGEKIT_LANG` 环境变量；机器可读字段保持英文，用户解释性输出支持中文 / 英文。
- Windows 收口验证修复：同步 PowerShell validation 的中文 README 匹配、非交互 review-needed 策略、items 按 action_id 查找、PYTHONIOENCODING 和版本链基线修复。

### 边界

- 不新增独立 review-needed / resolve-review-needed 命令。
- 不新增 --force。
- 不自动覆盖用户文件。
- 不自动 merge 或 commit。
- 不自动修改 workspace-map。
- 不自动创建 Project Capsule。
- 不引入外部 i18n 依赖。

---

## [0.43.0] - Minimal Project Capsule Bootstrap

### 问题背景

v0.41 已经定义了多项目工作区、工作区地图、`workspace-only` / `project-capsule` profile、最小 Project Capsule 模板和只读检查器。v0.41.1 又补齐了渐进语义：项目可以先登记为 `workspace-only`，继续由总工作区文档管理；只有明确切换为 `project-capsule` 后，才要求存在项目局部文档。

但这时还缺最后一步：用户已经决定某个 project 需要局部文档，也已经把它设置为 `project-capsule`，却仍然要手工复制模板。这个过程容易写错路径、把 repo / artifact / archive 当成 docs path，或者误复制完整 ForgeKit 协议副本。

### 设计结论

v0.43.0 只补齐 v0.41 scoped docs 设计中被延后的最小创建动作。

这版的核心结论是：

- **只处理用户已明确选择的单个 project**：ForgeKit 不替用户决定哪个项目需要局部文档。
- **只创建最小 Project Capsule**：不复制完整 ForgeKit，不生成 Repo Lite，不拆分现有工作区文档。
- **先 plan，再确认 apply**：写入前必须展示目标 project、profile、docs path 和文件动作。
- **路径安全优先**：任何越界、repo/artifact/archive 混用、完整协议副本迹象都必须 blocking。

### Added

- 新增 `scripts/bootstrap-project-capsule.py`。
- 新增 `project-template/scripts/bootstrap-project-capsule.py`。
- 新增单 project 创建入口：
...
- `plan` 输出 project id、docs profile、docs path、将创建的文件、已存在文件、review-needed 文件和 blocking 项。
- `apply --confirm` 只创建缺失的最小 Project Capsule 文件。
- 新增 v0.42.0 -> v0.43.0 safe migration，用于安装 bootstrap 脚本、更新 scoped docs / usage playbook，并登记 `minimal_project_capsule_bootstrap` feature。

### Changed

- scoped docs 协议补充 Project Capsule Bootstrap 流程。
- usage playbook 增加“创建子项目文档包”的提示词。
- AGENTS / CLAUDE / Codex rules 增加短规则：Project Capsule Bootstrap 必须先 plan，apply 需要显式确认，不能拆分 workspace docs 或迁移任务。
- README / README.en 增加最小入口提示，但不把 bootstrap 写成自动拆文档能力。

### Safety

- `.forgekit/workspace-map.json` 不存在或未启用时，不写文件，只输出 not-enabled / adoption guidance。
- project 为 `workspace-only` 时，不创建 capsule，输出 skipped，不 warning，不 blocking。
- project 为 `project-capsule` 时，必须通过 docs path 安全检查。
- 以下情况会 blocking：
  - 绝对路径；
  - `..` 越界；
  - `.git/**`；
  ...
  - 指向 archive；
  - 指向 repo_path 或其内部；
  - 指向 artifact path 或其内部；
  - 指向 `_template` 本身；
  - 与其他 project capsule 路径冲突；
  - 目标目录疑似完整 ForgeKit 副本。
- 已存在且内容相同的文件 no-op。
- 已存在但内容不同的文件标记 review-needed，不覆盖。
- 写入前先完成全量 preflight，避免部分创建。

### 用户可感知变化

- 用户把某个 project 切换为 `project-capsule` 后，可以用一个 plan/apply 流程创建最小局部文档包。
- ForgeKit 会清楚展示将写入哪些文件，不会偷偷拆分现有工作区文档。
- `workspace-only` 项目可以继续只登记边界，不会被要求创建 capsule。
- 创建完成后，可以继续运行 workspace checker 验证边界是否正确。
...
- 不自动启用 workspace map。
- 不自动生成 workspace map。
- 不自动把 `workspace-only` 改成 `project-capsule`。
- 不拆分 workspace docs。
- 不迁移 Source / Task / Risk / Test 记录。
- 不复制 workspace source 原文。
- 不生成 Repo Lite。
- 不移动业务 repo、artifact、archive 或报告证据目录。
- 不自动 commit、tag、push 或 PR。
- 不新增 runner、daemon、scheduler 或后台自动化。
...
真实使用中，ForgeKit managed docs 的写回容易走向两个极端。

一种是太频繁：每个小改、每次试错、每条临时命令都更新治理文档，导致文档重复、噪声变多、token 浪费。另一种是太少：长会话推进很久都不写，直到 compact、clear、换会话或一天结束后，才发现当前任务状态、验证结论、风险和下一步没有落盘。

同时，README 更适合解释定位、快速开始和常用入口，不适合承载大量日常提示词变体。用户真正需要的是：遇到初始化、接手、升级、提交前检查、归档、handoff、多项目只读分析等日常场景时，有一套可以直接复制给 AI 的短提示词。

### 设计结论

v0.42.0 将“什么时候写文档”和“日常怎么问 AI”从零散规则收敛成两个轻量协议。

这版的核心结论是：

- **文档写回按事件触发，不按每个小改触发**。
- **只写已确认的最小事实**：状态、验证、风险、下一步和必要证据路径，而不是聊天全文或长日志。
- **compact / clear / 换会话前必须留恢复点**：可预见中断前做 pre-compact checkpoint；不可预见 auto compact 后先做 post-compact recovery check。
- **README 保持短入口，详细提示词放 playbook**：README 只保留每个高频场景的一句话；完整变体放 `.forgekit/docs/usage-playbook.md`。

### Added

- 新增 `.forgekit/docs/work-session-checkpoint.md`，定义：
  - Micro Update；
  - Checkpoint Update；
  - Ship Update；
  - 写回触发条件；
...
- 新增 `.forgekit/docs/usage-playbook.md`，提供日常可复制提示词，覆盖：
  - 初始化新项目；
  - 接手已有项目；
  - 更新项目中的 ForgeKit；
  - 开始当天工作；
  ...
  - 第一性原理分析；
  - 对抗式审查。
- 新增 v0.41.1 -> v0.42.0 safe migration，用于安装 work-session checkpoint、usage playbook，并登记对应 feature。

### Changed

- workflow router 增加 work-session checkpoint 路由。
- context continuity 协议补充 pre-compact / post-compact 的具体触发方式。
- 文档职责矩阵明确 work-log、task-board、testing、risk-register、CHANGELOG 和 source-links 的最小写回边界。
- AGENTS / CLAUDE / Codex rules 明确：Micro Update 只是不写 ForgeKit governance docs，不限制授权范围内的业务代码、业务 README、注释、测试或配置修改。
- README / README.en / usage.html 调整为更短的常用入口；详细提示词变体集中到 usage playbook。

### 用户可感知变化

- 用户可以直接说“做一次压缩前进展保存”，让 AI 把当前目标、结论、风险、验证和下一步写到合适位置。
- compact 或换会话后，AI 有固定恢复动作：先只读恢复目标、最近结论、风险、验证和下一步，不急着改文件。
- 日常使用 ForgeKit 时，不需要每次重新组织长提示词，可以从 usage playbook 或 README 常用表格复制。
- 文档不再因为小试错被频繁污染，也不会在长会话结束时完全没有恢复点。
...
- 不做后台写回。
- 不写入未确认推断。
- 不把完整日志、长输出或临时推理写入常驻治理文档。
- 不自动 commit、tag、push、PR 或 archive apply。
- 不做 Project Capsule Bootstrap。
- 不生成 Repo Lite。
- 不新增 runner、daemon、scheduler 或后台自动化。



## [0.41.1] - Scoped Docs Profile & Workspace Git Warning Hotfix

### 问题背景

v0.41.0 建立了 multi-project / scoped docs 的基础模型，但在真实项目副本 `guanan` 上做验收时暴露出两个边界缺口。

第一个问题是检查器漏掉了 WorkspaceRoot 自身的 Git 状态。`check-workspace-integrity.py` 会检查 map 中列出的 repo 是否是 Git repo，却没有提醒 WorkspaceRoot 本身不是 Git repo。对于 `guanan` 这类“根目录放 ForgeKit 文档，子目录才是各业务 Git repo”的项目，这不应该阻断工作，但应该给出 warning：根级 `.forgekit` 文档本身可能没有随任何 repo commit / push。

第二个问题是 profile 语义不够细。v0.41.0 里 project 主要使用 `project-capsule`，启用 map 后如果声明了多个 project，检查器会要求所有 project 的 capsule 都存在。这个行为对“已经承诺使用 capsule 的 project”是合理的，但它缺少一种合法的渐进状态：**project 已经登记进 workspace map，但当前事实仍由 Workspace Docs 管理，暂时不创建 capsule**。

真实使用中，这个状态很常见。比如一个跨后端、前端、网关、扫描器的任务，当前仍由 workspace `task-board.md` 和 `testing.md` 统一管理更清晰；如果为了通过检查器而强行给所有 project 创建 capsule，反而会造成文档碎片化。

### 设计结论

v0.41.1 将 v0.41.0 的 profile 语义补齐，并修复 WorkspaceRoot Git warning 漏检。

这版的核心结论是：

* **WorkspaceRoot 非 Git 是 warning，不是 failure**：ForgeKit 应提醒用户根级治理文档可能没有进入版本控制，但不能要求所有 workspace root 都必须是 Git repo。
* **`workspace-only` 是合法 project profile**：project 可以先登记在 workspace map 中，但继续由 Workspace Docs 管理事实，不要求 project capsule 存在。
* **`project-capsule` 才表示必须有 capsule**：只有当 project 明确使用 `docs_profile: project-capsule` 时，缺少 `docs_path` 才是 blocking。
* **profile 必须按位置使用**：workspace、project、repo 的 profile 不能混用；`repo-lite` 不能写在 project 上，`project-capsule` 不能写在 repo 上。
* **migration 不能覆盖用户配置**：`.forgekit/workspace-map.json` 是用户项目边界配置，升级不能覆盖；checker 和协议文档只有在仍匹配 v0.41.0 基线时才可安全替换。

### Added

* 新增 `workspace_root_not_git` warning：

  * scoped docs 启用后，如果 WorkspaceRoot 不是 Git repo，普通模式只 warning；
  * `--strict` 下 warning 会导致退出码 1；
  * not-enabled 模式不执行该检查，避免旧项目被误伤。
* 新增 `project.docs_profile: workspace-only`：

  * project 已登记在 workspace map 中；
  * 当前事实仍由 Workspace Docs 管理；
  * 不要求 `.forgekit/projects/<project-id>/` 存在；
  * 不调用 capsule 完整性检查；
  * 不产生 `capsule_missing`。
* 新增 profile 使用位置校验：

  * workspace 使用 `workspace-full`；
  * project 只能使用 `workspace-only` 或 `project-capsule`；
  * repo 只能使用 `repo-lite`。
* 新增 v0.41.1 回归测试，覆盖 WorkspaceRoot 非 Git warning、`workspace-only` 缺 capsule、`project-capsule` 缺 capsule、profile 混用和 strict / require-enabled 退出码。

### Changed

* `project-capsule` 的语义被收紧为“该 project 应当拥有最小 Project Capsule”；缺少 `docs_path` 继续 blocking。
* `workspace-only` 成为从单 workspace docs 过渡到 project capsule 的正式中间状态。
* `check-workspace-integrity.py` 的 normal / strict 行为更明确：

  * warning 默认退出码 0；
  * `--strict` 下 warning 退出码 1；
  * blocking 退出码 1；
  * runtime error 退出码 2；
  * not-enabled 默认退出码 0；
  * `--require-enabled` 下 not-enabled 退出码 1。
* scoped docs 文档补充 docs profile 语义表，明确哪些 profile 可以用在哪一层。
* v0.41.1 migration 只替换仍匹配 v0.41.0 基线的 checker、协议文档和必要模板；用户改过的文件标记 `skipped-existing-review-needed`，不覆盖，也不宣称完整修复。

### Fixed

* 修复 WorkspaceRoot 非 Git repo 不产生 warning 的问题。
* 修复启用 workspace map 后无法表达“项目已登记但仍由 Workspace Docs 管理”的渐进状态问题。
* 修复 profile 可被混用导致 project / repo 语义边界不清的问题。
* 修复升级报告可能让用户误以为被修改过的 checker 已自动更新的问题；checksum 不匹配时明确提示 review-needed。

### 用户可感知变化

* 用户可以在 workspace map 中先登记多个 project，并把它们设置为 `workspace-only`，用于表达边界但不立刻创建 capsule。
* 当项目真的需要独立局部文档时，再把对应 project 改成 `project-capsule`，由检查器要求 capsule 存在。
* 如果 WorkspaceRoot 不是 Git repo，ForgeKit 会提醒根级 `.forgekit` 文档可能没有被版本控制，但不会阻断多 repo 工作流。
* 用户运行 `--strict` 时，可以把这类 warning 提升为失败，用于更严格的 CI 或发布前检查。
* 升级到 v0.41.1 时，用户已有的 `workspace-map.json` 不会被覆盖。

### 边界

* 不自动创建真实 Project Capsule。
* 不自动把 project 从 `workspace-only` 改成 `project-capsule`。
* 不自动拆分 workspace docs。
* 不迁移 Source / Task / Risk / Test 记录。
* 不生成 Repo Lite。
* 不移动业务代码、repo、artifact 或 archive。
* 不自动启用 multi-project。
* 不覆盖 `.forgekit/workspace-map.json`。
* 不要求 WorkspaceRoot 必须变成 Git repo；该检查只是可追溯性 warning。
* 不做 v0.42 的 capsule bootstrap、状态聚合、repo dashboard 或自动写回。
...
v0.40.2 之后，ForgeKit 已经能保护 active current docs，避免 archive 保存了历史却破坏当前任务链路。但真实项目继续推进后，又出现了一个更结构性的问题：**一个工作目标不一定等于一个 Git 仓库，也不一定只包含一个业务项目**。

在公司项目和独立开发中，这个问题都很常见：

* 一个交付目标可能同时涉及后端、前端、网关、扫描器、报告样本和运行证据。
* Git commit 必须分别进入各自 repo，但任务状态往往是跨 repo 的。
* “后端完成”不代表整体任务完成；前端、扫描器、联调数据和报告验证可能仍在等待。
* 如果所有项目细节都写进 workspace `task-board.md`、`testing.md`、`risk-register.md`，文档会越来越长。
* 如果每个 repo 都完整初始化一套 ForgeKit，又会复制几十个 managed docs，造成事实重复、升级困难和 token 浪费。
* report、runtime-archive、build output 这类目录是证据位置，不是 repo；但 AI 很容易把它们和代码仓库混在一起。
* archive 是历史证据，不是 current truth；Project Capsule 也不能变成完整 ForgeKit 副本。

这些问题的根因是：ForgeKit 之前的 ProjectRoot 模型默认“一个项目一套 current docs”，但真实工程里需要同时表达 **workspace、project、repo、artifact、archive** 五类边界。

### 设计结论

v0.41.0 引入 Multi-Project Workspace & Scoped Docs Protocol，用于在一个 ForgeKit workspace 下管理多个 project / repo / artifact 的边界。

这版的核心结论是：

* **Workspace Docs 管跨项目事实**：跨 project 的 source、overall task status、E2E / integration matrix、跨项目风险和工作日志仍属于 `.forgekit/docs/**`。
* **Project Capsule 管局部事实**：当某个 project 长期独立推进时，可以使用 `.forgekit/projects/<project-id>/` 保存局部任务、局部风险、局部测试和项目决策。
* **Repo Lite 只指向代码边界**：repo 可以有薄入口和本地命令说明，但不能成为第三套治理事实源。
* **Artifact 只是证据位置**：报告样本、测试输出、运行日志、构建物和下载材料只能作为 evidence 被 current docs 引用，不能被当成 repo 或 project capsule。
* **Archive 不参与 current truth**：`.forgekit/archive/**` 只保存历史，不可作为 workspace docs 或 project docs path。
* **机器索引用 JSON，不用 Markdown 表格或 YAML 子集**：`.forgekit/workspace-map.json` 是脚本读取的边界地图；Markdown 继续负责人和 AI 阅读的协议、任务和说明。
* **v0.41 建立协议和检查器，不搬家**：不自动拆现有文档，不创建真实 project capsule，不生成 repo-lite，不移动业务代码或证据目录。

### Added

* 新增 `.forgekit/workspace-map.json`，作为机器可读的 workspace 边界地图，用标准 JSON 描述：
  ...
  * read scope；
  * 路径和引用关系。
* 新增 `.forgekit/docs/scoped-docs.md`，定义 WorkspaceRoot、ProjectScope、RepoRoot、ArtifactRoot、ArchiveRoot、Workspace Docs、Project Capsule 和 Repo Lite 的职责边界。
* 新增最小 Project Capsule 模板：
...
* 新增只读检查器 `check-workspace-integrity.py`，用于检查：

  * workspace map 是否启用和合法；
  * JSON schema 和路径安全；
  * Project / Repo / Artifact ID 引用；
  * repo_path、docs_path、artifact path 是否混用；
  * artifact / archive 是否被误配置为 repo 或 current docs；
  * Project Capsule 是否过重；
  * capsule 是否复制完整 ForgeKit docs 或治理入口；
  * profile、scope 和路径关系是否一致。
* 新增 `--strict`、`--json`、`--scope`、`--project`、`--require-enabled` 等检查入口。
* 新增 v0.40.2 -> v0.41.0 safe migration，用于安装 disabled map、scoped docs、Project Capsule `_template`、workspace checker 和相关入口规则。

### Changed

* 现有 ProjectRoot 在 v0.41 中作为 WorkspaceRoot 的兼容别名，不强制重命名旧字段或迁移旧项目结构。
* Multi-project scoped docs 默认 available 但 disabled：

  * legacy / single-project 项目没有 map 时，只输出 adoption guidance；
  * 默认 `.forgekit/workspace-map.json` 为 `enabled: false`；
  * 安装模板不代表用户已经启用 multi-project。
* workflow-router 增加 scope selection：

  * 先判断 single-project 还是 multi-project；
  * multi-project 中先选择 workspace / project / repo scope；
  * 未启用时继续使用既有 single-project 流程。
* `check-current-docs-integrity.py` 与 `check-workspace-integrity.py` 职责分离：

  * current-docs checker 检查单作用域 Source / Task / Risk / Testing 链路；
  * workspace checker 检查跨作用域结构、路径、profile、artifact/repo/archive 边界；
  * 两者不能互相替代。
* ID 规则继续兼容 `SRC-*`、`TASK-*`、`RISK-*`、`TEST-*`；v0.41 不强制引入 `WTASK-*` 或 `PTASK-*`。
* Project 状态和 workspace overall status 分离：局部完成不自动等于整体完成。

### 用户可感知变化

* 用户可以在一个 ForgeKit workspace 中描述多个 repo，例如 backend、frontend、gateway、scanner，同时保持各 repo 的 Git push 边界独立。
* 用户可以把 `project/report`、`runtime-archive`、测试报告、导出文件等目录明确标记为 artifact，避免 AI 把证据目录误当成代码 repo。
* 用户可以先只启用 map 来表达边界，而不立刻拆分 workspace docs。
* Project Capsule 有了固定的最小模板，后续需要项目局部文档时，不必复制完整 ForgeKit。
* 运行 workspace checker 后，用户能看到路径越界、repo/artifact 混用、archive 被当 current docs、capsule 过重等问题。
* 旧单项目工作流不会因为没有 map 而失败；未启用 multi-project 时只得到 adoption guidance。

### 边界

* 不自动启用 multi-project scoped docs。
* 不自动拆分现有 `task-board.md`、`risk-register.md`、`testing.md`、`work-log.md` 或 `task-intake.md`。
* 不自动创建真实 `.forgekit/projects/<project-id>/`。
* 不自动生成 Repo Lite。
* 不移动业务 repo、artifact、archive 或报告证据目录。
* 不把 artifact 变成 project 或 repo。
* 不把 archive 变成 current truth。
* 不引入 YAML runtime parser；YAML 只能作为示例或说明，不参与检查器运行时判断。
* 不强制迁移旧 Source / Task / Risk / Test ID。
* 不自动 commit、push、PR 或 tag。
* 不新增 runner、daemon、scheduler、UI、文档站点或后台任务。
...
## [0.40.2] - Active Current Docs Integrity Guard

### 问题背景

v0.39 引入 Project Maintenance Operations 和 Archive Capsule 后，ForgeKit 已经能把旧材料移出默认上下文，并生成 summary、manifest、index、item log 等归档证据。v0.40.1 又修复了升级重跑和入口命令问题。但真实项目 `guanan` 的升级 / 归档暴露出一个更严重的安全边界缺口：**archive 证明了历史被保存，却没有证明 current docs 仍能支撑未完成任务继续工作**。

这类问题在活跃任务场景下尤其危险：

- 当前 V1.3 任务还没有结束，`task-board.md` 里仍有 `Review`、`Waiting`、`Backend Ready`、`Needs Fix` 等状态。
- `task-board.md` 仍引用真实 `Source ID`，但 `task-intake.md` 可能被压成模板，导致任务无法反查原始来源。
- `risk-register.md`、`traceability.md`、`testing.md` 可能被重置为模板或过度压缩，导致当前风险、追踪链和验证基线只剩在 archive 里。
- archive summary 看起来完整，但后续 AI 如果只读 current docs，会失去判断任务是否跑偏、风险是否仍有效、验证是否已完成的依据。
- 用户说“归档一下”时，AI 可能把 active work cleanup 误写成 completed phase archive。

这些问题的共同根因是：v0.39 的 archive 更关注“历史有没有保存”，但没有把 **current docs integrity** 作为 archive / maintenance 成功条件。

### 设计结论

v0.40.2 将 current docs integrity 明确为 archive / maintenance 的硬保护条件。

这版的核心结论是：

- **current docs 是当前工作状态数据库**：未完成任务的来源、任务、风险、追踪和验证基线必须能从 `.forgekit/docs/**` 恢复，不能只存在于 archive。
- **archive 是历史证据，不是 current truth**：archive 可以保存旧全文、旧日志和旧验证材料，但不能替代当前 `task-intake`、`task-board`、`risk-register`、`traceability` 和 `testing`。
- **archive 成功必须同时满足两个条件**：历史材料可找回；current docs 仍能支撑 active tasks。
- **活跃任务不能被写成 completed phase archive**：如果 task-board 还有 active tasks，本次归档只能被标记为 legacy transition snapshot、provisional archive、evidence snapshot 或 active-work cleanup snapshot。
- **检查失败时先恢复当前态，不继续归档**：Source 断链、风险丢失、追踪矩阵模板化、测试基线缺失时，应进入 Current State Restoration Pass，而不是继续移动文件或写“归档完成”。

### Added

- 新增 `.forgekit/docs/current-docs-integrity.md`，定义 Current Docs Invariants、Active Work Guard、Archive Preflight / Postflight、Restoration Guidance、Template Placeholder Rules 和边界。
- 新增只读检查脚本 `scripts/check-current-docs-integrity.py`，并同步到 project template。
- 新增 current docs integrity 检查项：
  - `task-board.md` 中真实 `Source ID` 必须能反链到 `task-intake.md`；
  - active `Task ID` 必须在 `traceability.md` 中有最小追踪链；
  - 有 active tasks 时，`task-intake.md`、`risk-register.md`、`traceability.md`、`testing.md` 不能只是模板占位；
  - 有 Review / Backend Ready / Submitted / Needs Fix 等任务时，`testing.md` 必须保留当前验证基线或明确 `TODO_REVIEW`；
  - archive / migration 相关 TODO_REVIEW 不能只留在 archive summary；
  - active work archive 不能被标记为 completed phase archive；
  - `work-log.md` 中被后续修正的 handed-off 记录必须有 superseded / corrected 说明。
- 新增 `--repo-root`、`--strict`、`--json` 参数。
- 新增退出码约定：
  - `0`：通过；
  - `1`：存在 blocking integrity failure；
  - `2`：运行错误或必需目录缺失。
- 新增 v0.40.1 -> v0.40.2 幂等 safe migration，用于安装 integrity 文档、检查脚本和相关 protocol 更新。
- 新增 smoke / regression 场景，覆盖 Source 断链、risk 模板化、traceability 模板化、testing 缺基线、示例 ID 误判、archive preflight 阻断、postflight 检查和 migration 幂等。

### Changed

- `archive-capsule.py` 在 plan / apply 前执行 current docs integrity preflight，并在 apply 后执行 postflight。
- archive apply 遇到 blocking integrity failure 时默认停止，不能只靠普通 `--confirm` 继续。
- archive postflight 失败时，输出 Current State Restoration guidance，不把本次 archive 写成 completed。
- `forgekit-project.py` / maintenance flow 输出 current docs integrity 状态：
  - `upgrade-sync` 可提示 integrity 问题，但不阻断 safe migration；
  - archive apply 必须受 integrity blocking 约束；
  - root 不是 Git 仓库时只作为 warning，不误判为 integrity failure。
- workflow-router 增加“归档后当前任务接不上”“task-intake 空了”“任务看板和原始任务对不上”“current docs 断链”“archive 后任务跑偏”“恢复当前状态”等路由，指向 current-docs-integrity check 和 Current State Restoration Pass。
- archive-capsule / project-maintenance 增加 Active Work Archive Guard：活跃任务未结束时，必须确认本次是 phase close、legacy transition snapshot、evidence snapshot 还是 cleanup snapshot。
- context-continuity 明确：current docs integrity failure 属于 Critical Fact，必须 checkpoint。
- reasoning-review 明确：从第一性原理审查 archive / maintenance 时，必须检查 current docs invariants 和 active work failure paths。
- AGENTS、CLAUDE 和 Codex rules 增加短规则：archive / maintenance 前后检查 current docs integrity；Source ID 必须能反链；active tasks 不得只存在 archive；current docs 断链时先恢复当前态。

### Fixed

- 修复 archive / maintenance 只验证历史保存、不验证 current state 可继续的问题。
- 修复 active tasks 仍存在时，archive 可能被误表述为 completed phase archive 的问题。
- 修复 `task-intake.md`、`risk-register.md`、`traceability.md`、`testing.md` 被模板化后仍可能继续 archive apply 的问题。
- 修复普通 `--confirm` 可以绕过 current docs 断链继续归档的问题。
- 修复示例 ID / 模板占位可能干扰真实 Source / Task 检查的问题。
- 修复 work-log 中旧 handed-off 记录和后续 needs-fix / restoration 状态冲突时缺少提示的问题。

### 用户可感知变化

- 用户可以直接运行 current docs integrity 检查：
...
- 归档前，ForgeKit 会先检查当前任务是否还能从 current docs 接上，而不是只检查 archive 是否能生成。
- 如果 `task-board.md` 里还有未完成任务，但 `task-intake.md`、`risk-register.md`、`traceability.md` 或 `testing.md` 断链，archive apply 会停止并提示恢复当前态。
- 活跃任务场景下，ForgeKit 会避免把整理 / 迁移误写成“阶段完成归档”。
- 归档后如果 current docs 被破坏，ForgeKit 会明确进入 Current State Restoration Pass，而不是继续写“handoff / completed”。

### 边界

- 不自动恢复 current docs；检查器只读，只报告问题和恢复建议。
- 不把 archive 全文复制回 current docs。
- 不覆盖 `task-intake.md`、`task-board.md`、`risk-register.md`、`traceability.md`、`testing.md` 或 business docs。
- 不自动移动、删除或重排 archive。
- 不自动提交、push、PR、tag。
- 不新增 runner、daemon、scheduler、后台任务或自动项目管理系统。
- 不把 root 非 Git 仓库当作 current docs integrity failure；它只作为可追溯性 warning。
- 不要求所有历史风险和测试流水都恢复到 current docs，只要求保留当前未完成任务继续需要的最小事实链。
...
v0.39 引入统一项目入口 `forgekit-project.py` 后，用户可以用一个入口处理 init、up-to-date、upgrade-sync、toolkit-too-old 和 legacy adoption。v0.40 又继续叠加了 reasoning-review migration。真实升级测试时暴露出两个发布级问题。

第一个问题是跨平台入口文档不够准确：ForgeKit 统一的是“入口语义”，不是所有平台都复制同一条命令。Windows、macOS、Ubuntu 对 Python 命令、路径分隔符和 shell wrapper 的习惯不同。如果 README 只写一条“统一命令”，macOS / Linux 用户容易遇到 `python` 不存在、路径写法不兼容或 wrapper 不明确的问题。

第二个问题更严重：safe migration 不够幂等。真实项目从 `0.38.0` 升到 `0.40.0` 时，plan 显示可以 safe apply，但 apply 遇到已经存在的 `.forgekit/docs/project-maintenance.md` 后直接失败。这个场景可能来自部分升级失败、用户手动复制过文件、旧会话提前写入文档，或者重复运行升级。safe migration 如果不能重跑，就无法承担“安全升级”的职责。

这些问题的共同根因是：v0.39 的统一入口和 v0.36 的 migration 模型在干净项目上成立，但对真实项目常见的“半升级、重复运行、目标文件已存在、跨平台命令差异”处理不够稳。

### 设计结论

v0.40.1 将这次修复收敛为两个 hotfix 原则：

- **统一入口不等于统一 shell 命令**：README 应分别给出 Windows PowerShell、macOS / Linux 的 Python 命令和薄 wrapper 命令；入口语义统一，平台命令分开。
- **safe migration 必须幂等、可重跑、可从部分失败恢复**：install-if-missing 类动作遇到已有文件时，不能直接失败；必须区分 missing、already-present、review-needed，并且 plan / apply 使用一致的目标状态判断。

这版的关键结论是：

- 目标文件不存在：safe action 可以复制。
- 目标文件存在且内容相同：视为 already-present / no-op。
- 目标文件存在但内容不同：保留用户文件，标记 review-needed / skipped-existing，不覆盖，也不中断后续 safe feature enable。
- plan 不能只读取静态 migration safety；必须结合目标项目当前状态，避免 Safe actions count 误导用户。
- 重复执行同一次升级，应得到 no-op / up-to-date，而不是 conflict。
- 用户已有入口规则、managed docs 和 business docs 优先于模板 payload。
...
- README、README.en、usage.html 和 template README 增加分平台统一入口示例：
  - Windows PowerShell Python 命令；
  - Windows PowerShell wrapper 命令；
  - macOS / Linux `python3` 命令；
  - macOS / Linux Bash wrapper 命令。
- 新增 migration 幂等性回归场景：
  - 目标文件已存在且内容相同；
  - 目标文件已存在且内容不同；
  - 部分升级失败后重跑；
  - 升级完成后再次运行显示 up-to-date；
  - wrapper dry-run。

### Changed

- `copy_file_if_missing` 统一处理已有目标文件：
  - `SAFE`：目标不存在，复制；
  - `ALREADY-PRESENT`：目标存在且内容相同，不写；
  - `REVIEW-NEEDED`：目标存在但内容不同，不覆盖，保留用户文件。
- plan / apply 共用目标状态分类，减少“plan 说 safe，apply 才冲突”的不一致。
- apply 遇到 `REVIEW-NEEDED` 时输出 skipped-existing / review-needed，但不中断后续 feature 登记和安全迁移。
- Safe actions count 只统计实际待执行动作，避免把 already-present 的 no-op 当作需要复制的 action。
- 统一入口继续作为薄编排层调用既有 init / upgrade 脚本，不复制 migration 逻辑。

### Fixed

- 修复 macOS / Linux README 中统一入口命令不清晰或不兼容的问题。
- 修复 v0.38.0 -> v0.40.0 升级时，目标 managed doc 已存在导致 safe apply 失败的问题。
- 修复部分升级失败后无法通过统一入口恢复的问题。
- 修复重复运行 upgrade-sync 可能再次写 state 或误报待升级的问题。
- 修复 plan / apply 对已有文件状态判断不一致的问题。
- 修复用户已有不同内容文档被模板 payload 视为 conflict 而中断升级的问题。

### 用户可感知变化

- Windows 用户可以继续使用 PowerShell 命令；macOS / Linux 用户会看到 `python3` 和 Bash wrapper 的明确写法。
- 用户可以在项目升级失败后重新运行统一入口，ForgeKit 会跳过已存在文件并继续完成剩余 safe migration。
- 已经安装过的文档和脚本不会被重复写入。
- 如果用户本地文件和模板内容不同，ForgeKit 会保留用户文件并提示 review，而不是覆盖或直接失败。
- 升级完成后再次运行统一入口，会显示 up-to-date，不再误写 state。
- 用户不需要手动删除 `.forgekit/docs/project-maintenance.md` 之类文件来绕过升级错误。

### 边界

- wrapper 只转发参数，不复制业务逻辑。
- 不删除 `scripts/forgekit-project.py`，也不替代既有 init / upgrade 脚本。
- 不重写 v0.36 migration 系统，只修正 install-if-missing 类动作的状态判断和幂等性。
- 不覆盖或删除用户已有文件。
- 不改 business docs。
- 不自动 commit、push、PR、tag。
- 不新增 runner、daemon、scheduler 或后台任务。
- 不把 review-needed 文件自动合并；只提示人工复核。
