...
v0.41 已定义 workspace map、`workspace-only` / `project-capsule` profile、最小 capsule 模板和只读 checker，但用户把单个 project 明确切换为 `project-capsule` 后，仍需手工复制模板。这个缺口容易造成多复制协议、写错 repo/artifact/archive 路径或覆盖已有局部事实。

### 设计结论

v0.43.0 只补一个单 project、plan-first、显式确认、幂等的最小创建动作。它不扩展 multi-project 模型，也不主动决定哪个 project 应使用 capsule。

### Added

* 新增 `bootstrap-project-capsule.py plan/apply --confirm`，只从 `_template` 创建六个最小文件。
* plan 输出 project、profile、docs path 和 create/already-present/review-needed/blocking 汇总。
* 新增 v0.42.0 到 v0.43.0 safe migration，安装脚本并登记 feature。

### Safety

* map 未启用或 project 为 `workspace-only` 时不写文件。
* 绝对路径、越界路径、repo/artifact/archive/Workspace Docs 冲突和完整 ForgeKit 协议副本均 blocking。
...
真实使用中，managed docs 的写回容易走向两个极端：每个小改都更新文档，产生重复和 token 浪费；或长时间不写，直到 compact、clear、换会话或一天结束后才发现任务状态、验证和风险没有落盘。与此同时，README 主要解释初始化和能力边界，用户不知道日常应该怎样用一句话触发正确流程。

### 设计结论

v0.42.0 将写回改为事件触发，并区分 Micro Update、Checkpoint Update 和 Ship Update。可预见的 compact/clear/换会话前执行 pre-compact checkpoint；不可预见的 auto compact 后先执行 post-compact recovery check。所有写回只保存已确认的最小事实，不复制聊天或长日志。

### Added

* 新增 `.forgekit/docs/work-session-checkpoint.md`，定义三种写回粒度、触发条件、写回位置和主动/被动 compact 处理。
* 新增 `.forgekit/docs/usage-playbook.md`，提供初始化、接手、升级、日常工作、checkpoint、提交前检查、归档、handoff 和多项目只读分析的可复制提示词。
* 新增 v0.41.1 到 v0.42.0 safe migration，只安装新协议并登记 feature。

### Changed

* workflow router、context continuity、文档职责矩阵和代码搜索入口增加 work-session checkpoint 路由。
* README / README.en / usage.html 只保留每个场景一条短提示词，详细解释集中在 usage playbook。
* AGENTS / CLAUDE / rules 明确：Micro Update 只是不写 ForgeKit governance docs，不限制授权范围内的业务代码、README、注释、测试或配置修改。

### Safety

* 未确认推断不得写成 current truth。
* report-only / review-only 不借 checkpoint 协议自动修复 current docs。
* 不做后台写回、自动 compact 检测、聊天全文扫描、文档自动拆分、Project Capsule Bootstrap 或 Repo Lite 生成。
* 不自动 commit、tag、push、PR、archive apply，也不新增 runner、daemon 或 scheduler。
