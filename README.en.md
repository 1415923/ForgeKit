...
ForgeKit is not an app framework, code scaffold, or deployment tool. It turns a new or existing repository into an AI-ready project for Codex, Claude Code, and adjacent coding tools: clear entry points, discovery interviews, project documents, stack rules loaded on demand, review gates, release checks, and safety boundaries.

In short: ForgeKit makes the assistant clarify the goal, plan, risks, and validation path before it starts coding.
