...
When upgrading a project generated by an older ForgeKit version, use the initializer in upgrade mode and do not use `-Force` / `--force`:
...
Upgrade mode only adds template files that are missing. Existing `docs/`, `.codex/`, `AGENTS.md`, `CLAUDE.md`, and init metadata are preserved by default, so project facts are not overwritten. Existing files that differ from the newer template are recorded in `.codex/upgrade-report.md` for human or AI-assisted merge review.

To inspect newer template content without overwriting existing files, export template copies:
...
Newer template copies are written under `.codex/upgrade-templates/`. Use `-Force` / `--force` only for disposable rebuilds or when you explicitly want template files overwritten.

After upgrading, ask the AI assistant to review `.codex/automation-decision.md`, `.codex/hooks.md`, `.agents/skills/README.md`, and `docs/代码库地图.md`, then decide which new capabilities should be merged into the current project facts.
