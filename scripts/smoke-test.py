...
def assert_manifest_lock(target):
    ...
    if lock.get("installed_version") != FORGEKIT_VERSION:
        fail(f"template-lock installed_version must be {FORGEKIT_VERSION}")
...
def assert_versioned_migration_upgrade(repo, target, temp_parent):
    ...
    if f"Status: current" not in check.stdout or f"Current version: {FORGEKIT_VERSION}" not in check.stdout:
        fail(f"versioned migration check did not report current {FORGEKIT_VERSION} state")
    ...
    if migrated.get("forgekit_version") != FORGEKIT_VERSION:
        fail(f"version chain did not update state to {FORGEKIT_VERSION}")
    ...
    if f"To: {FORGEKIT_VERSION}" not in apply_plan.stdout or (apply_target / ".forgekit" / "migration-proof.txt").exists():
...
def assert_current_docs_integrity(repo, target, temp_parent):
    ...
    if migrated.get("forgekit_version") != FORGEKIT_VERSION or migrated["features"].get("active_current_docs_integrity_guard") is not True:
        fail(f"migration chain did not update state through {FORGEKIT_VERSION}")
