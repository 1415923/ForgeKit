...
function Test-AIEngineeringLoop {
    ...
    foreach ($removedLoopDoc in @(
        "project-template\docs\loop-readiness.md",
        "project-template\docs\loop-blueprint.md",
        "project-template\docs\loop-operations.md"
    )) {
        if (Test-Path -LiteralPath (Join-Path $repoRoot $removedLoopDoc)) {
            Add-Error "Removed loop document still exists: $removedLoopDoc"
        }
    }
    Test-RequiredPattern "project-template\governance\ai-engineering-loop.md" "DIAGNOSTIC" "Execution intent diagnostic"
    Test-RequiredPattern "project-template\governance\ai-engineering-loop.md" "SMOKE" "Execution intent smoke"
    Test-RequiredPattern "project-template\governance\ai-engineering-loop.md" "FORMAL" "Execution intent formal"
    Test-RequiredPattern "project-template\governance\ai-engineering-loop.md" "## Formal Evidence Retention" "Formal evidence heading"
    Test-RequiredPattern "project-template\governance\ai-engineering-loop.md" "## Formal Authority Chain" "Formal authority heading"
    Test-RequiredPattern "project-template\governance\ai-engineering-loop.md" "Repeated governance-only blocking without new mainline evidence" "Governance stop-loss"
    Test-RequiredPattern "project-template\governance\agent-entry-contract.md" "## Effect Risk" "Effect risk contract"
    Test-RequiredPattern "project-template\governance\agent-entry-contract.md" "## Bounded Effect Envelope" "Bounded effect envelope"
    ...
    Test-RequiredPattern "project-template\docs\bounded-auto-loop-policy.md" "不是 readiness questionnaire、mandatory blueprint/state、runner、daemon、cron、scheduler、多 agent dispatcher、自动 PR 或 worktree orchestration" "Bounded auto no runner boundary"
    ...
    Test-RequiredPattern "project-template\docs\maker-checker-protocol.md" "Impact Severity: CRITICAL `| MAJOR `| MINOR `| NOTE" "Maker checker impact severity"
    Test-RequiredPattern "project-template\docs\maker-checker-protocol.md" "Blocking: YES `| NO" "Maker checker blocking field"
    Test-RequiredPattern "project-template\docs\maker-checker-protocol.md" "checker nonzero != project Blocking" "Maker checker exit boundary"
    ...
    Test-RequiredPattern "project-template\.forgekit\docs\document-responsibility.md" "Document ownership does not imply mandatory population" "Document responsibility fact trigger"
    Test-RequiredPattern "project-template\docs\current-docs-integrity.md" "Blocking=NO" "Current docs no-risk case"
    Test-RequiredPattern "project-template\docs\current-docs-integrity.md" "active task alone cannot block" "Current docs no-testing case"
    Test-RequiredPattern "project-template\docs\current-docs-integrity.md" "closure/handover/ship" "Current docs closure writeback case"
    Test-RequiredPattern "project-template\docs\project-maintenance.md" "One-Time Cleanup Plan" "Maintenance one-time cleanup"
    Test-RequiredPattern "project-template\docs\project-maintenance.md" "Proposed Action:" "Maintenance cleanup proposed action"
    Test-RequiredPattern "project-template\docs\project-maintenance.md" "不建立 ``CURRENT/HISTORICAL`` 等持久 taxonomy" "Maintenance no persistent taxonomy"
    ...
    Test-RequiredPattern "README.md" "独立代码审查" "Root README current review capability"
    ...
    Test-RequiredPattern "project-template\changes\_template\proposal.md" "## 信任边界" "proposal trust boundary"
    Test-RequiredPattern "project-template\changes\_template\proposal.md" "## 阶段授权" "proposal stage authorization"
    Test-RequiredPattern "project-template\changes\_template\verification.md" "Frozen Acceptance Matrix" "frozen acceptance matrix"
    Test-RequiredPattern "project-template\changes\_template\review.md" "ReviewMode: initial | blocker-recheck" "bounded re-review mode"
    Test-RequiredPattern "project-template\docs\maker-checker-protocol.md" "Initial Review and Recheck" "review convergence protocol"
    Test-RequiredPattern "project-template\.forgekit\docs\usage-playbook.md" "## 8.1 Maker 实现" "maker short prompt"
    Test-RequiredPattern "project-template\.forgekit\docs\usage-playbook.md" "## 8.2 Reviewer 首审" "initial reviewer short prompt"
    Test-RequiredPattern "project-template\.forgekit\docs\usage-playbook.md" "## 8.3 Reviewer 限定复审" "re-review short prompt"
    Test-RequiredPattern "project-template\.forgekit\docs\usage-playbook.md" "本轮修复新引入且违反冻结合同" "fix-introduced re-review boundary"
    Test-RequiredPattern "project-template\.forgekit\docs\usage-playbook.md" "同一个交互会话" "single-command interactive upgrade"
    Test-RequiredPattern "scripts\forgekit-project.py" "Upgrade result summary" "unified upgrade result summary"
    Test-RequiredPattern "scripts\forgekit-project.py" "AGENTS_MAKER_CHECKER_SNIPPET" "AGENTS manual merge snippet"
    Test-RequiredPath "migrations\0.44.0\migration.json"
    Test-RequiredPath "project-template\migrations\0.44.0\migration.json"
    Test-RequiredPattern "migrations\0.44.0\migration.json" '"target": ".forgekit/changes/_template/proposal.md"' "safe change template migration target"
    Test-RequiredPattern "project-template\migrations\0.44.0\migration.json" '"target": ".forgekit/changes/_template/proposal.md"' "template safe change migration target"
    Test-RequiredPattern "migrations\0.44.0\migration.json" '"target": "scripts/forgekit-upgrade.py"' "advanced upgrade cancel contract migration"
    Test-RequiredPattern "migrations\0.44.0\migration.json" "AGENTS manual-merge snippet" "AGENTS discovery action"
}
...
function Test-TemplateManifest {
    ...
    if (Test-Path -LiteralPath $manifestPath) {
        ...
        if ($manifest.template_version -ne $forgekitVersion) {
...
function Test-HarnessEntryConsistency {
    ...
    Test-RequiredPattern "project-template\.forgekit\state.json" "`"forgekit_version`": `"$forgekitVersion`"" "State ForgeKit version (must match VERSION)"
...
function Test-PluginDistribution {
    ...
    if ($codexPluginJson.version -ne $forgekitVersion) {
        Add-Error "Root Codex plugin version ($($codexPluginJson.version)) does not match VERSION ($forgekitVersion)"
    }
    ...
    if ($claudePluginJson.version -ne $forgekitVersion) {
        Add-Error "Root Claude plugin version ($($claudePluginJson.version)) does not match VERSION ($forgekitVersion)"
    }
...
function Test-IndependentCodeReviewProtocol {
    ...
    Test-RequiredPattern "project-template\docs\maker-checker-protocol.md" "代码修改、文件数量、脚本修改或 bounded-auto 本身不机械产生 independent-review gate" "Independent review trigger"
...
function Test-ContextContinuityProtocol {
    ...
    Test-RequiredPattern "README.md" "上下文连续性" "README context continuity entry"
...
function Test-ReasoningReviewProtocol {
    ...
    Test-RequiredPattern "project-template\docs\bounded-auto-loop-policy.md" "Blocking=YES" "Bounded-auto adversarial stop gate"
    Test-RequiredPattern "project-template\docs\context-continuity.md" "Blocking=YES" "Context Critical Facts finding"
...
function Test-MultiProjectScopedDocs {
    ...
    Test-RequiredPattern "project-template\.forgekit\workspace-map.json" '"docs_profile": "workspace-full"' "Workspace full profile default"
    ...
    Test-RequiredPattern "migrations\0.41.1\migration.json" '"from": "0.41.0"' "v0.41.1 migration source"
    Test-RequiredPattern "migrations\0.41.1\migration.json" '"to": "0.41.1"' "v0.41.1 migration target"
    Test-RequiredPattern "migrations\0.41.1\migration.json" '"replace_file_if_baseline_matches"' "v0.41.1 guarded replacement action"
    Test-RequiredPattern "migrations\0.41.1\migration.json" "skipped-existing-review-needed" "v0.41.1 skipped checker warning"
